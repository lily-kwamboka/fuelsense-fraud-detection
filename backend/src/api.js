...
async function checkExpiredSubscriptions() {
  try {
    ...
    if (result.rows.length) {
      ...
      result.rows.forEach(row => { console.log(`  - Station ID: ${row.station_id}`); });
    }
  } catch (err) {
...
app.get('/api/tanks', async (req, res) => {
  try {
    ...
    let query = `
      SELECT t.id, t.tank_number, t.fuel_type, t.capacity_litres, t.fuel_density_at_15c,
             t.low_stock_threshold_pct, s.name AS station_name, s.id AS station_id,
             r.innage_mm, r.water_mm, r.temperature_c, r.nsv_litres, r.vcf, r.recorded_at,
             ROUND((r.nsv_litres / t.capacity_litres) * 100, 1) AS fill_pct
       FROM tanks t
       JOIN stations s ON s.id = t.station_id
       LEFT JOIN LATERAL (
         SELECT * FROM atg_readings WHERE tank_id = t.id ORDER BY recorded_at DESC LIMIT 1
       ) r ON TRUE`;
    const params = [];
    if (stationId) {
      ...
      const userRes = await client.query(`SELECT role, station_id FROM user_profiles WHERE supabase_uid = $1`, [supabaseUid]);
...
app.get('/api/user-profile', async (req, res) => {
  try {
    ...
    const result = await client.query(
      `SELECT u.*, s.name AS station_name FROM user_profiles u LEFT JOIN stations s ON s.id = u.station_id WHERE u.supabase_uid = $1`,
      [supabaseUid]
    );
...
app.get('/api/tanks/:id/readings', async (req, res) => {
  try {
    const client = await getDb();
    const result = await client.query(
      `SELECT innage_mm, nsv_litres, temperature_c, water_mm, vcf, recorded_at FROM atg_readings WHERE tank_id = $1 ORDER BY recorded_at DESC LIMIT 60`,
      [req.params.id]
    );
    res.json(result.rows);
  } catch (err) {
...
app.get('/api/deliveries', async (req, res) => {
  try {
    const client = await getDb();
    const result = await client.query(
      `SELECT d.id, d.status, d.supplier_name, d.bol_number, d.bol_nsv_litres, d.received_nsv_litres,
              d.variance_litres, d.variance_pct, d.variance_classification, d.tolerance_pct,
              d.truck_arrived_at, d.stabilisation_at, t.tank_number, t.fuel_type
       FROM deliveries d JOIN tanks t ON t.id = d.tank_id
       ORDER BY d.truck_arrived_at DESC NULLS LAST LIMIT 20`
    );
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    const result = await client.query(
      `SELECT d.*, t.tank_number, t.fuel_type,
              o.innage_mm AS opening_innage_mm, o.temperature_c AS opening_temp, o.nsv_litres AS opening_nsv, o.recorded_at AS opening_recorded_at,
              c.innage_mm AS closing_innage_mm, c.temperature_c AS closing_temp, c.nsv_litres AS closing_nsv, c.recorded_at AS closing_recorded_at
       FROM deliveries d
...
app.post('/api/deliveries', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const readingRes = await client.query(`SELECT id, nsv_litres FROM atg_readings WHERE tank_id = $1 ORDER BY recorded_at DESC LIMIT 1`, [tank_id]);
    ...
    const delRes = await client.query(
      `INSERT INTO deliveries (tank_id, supplier_name, bol_number, bol_nsv_litres, truck_arrived_at, opening_reading_id, status) VALUES ($1, $2, $3, $4, NOW(), $5, 'in_progress') RETURNING id`,
...
app.get('/api/reconciliation', async (req, res) => {
  try {
    const client = await getDb();
    const result = await client.query(
      `SELECT r.recon_date, r.opening_nsv, r.closing_nsv, r.deliveries_nsv, r.pump_sales_litres,
              r.theoretical_closing, r.variance_litres, t.tank_number, t.fuel_type
       FROM daily_reconciliation r JOIN tanks t ON t.id = r.tank_id
       ORDER BY r.recon_date DESC, t.tank_number LIMIT 60`
    );
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post('/api/reconciliation/pump-sales', async (req, res) => {
  ...
  try {
    ...
    if (!openRes.rows.length || !closeRes.rows.length) return res.status(400).json({ error: 'No readings found for this tank on this date' });
    ...
    await client.query(
      `INSERT INTO daily_reconciliation (tank_id, recon_date, opening_nsv, closing_nsv, deliveries_nsv, pump_sales_litres, theoretical_closing, variance_litres)
       ...
       ON CONFLICT (tank_id, recon_date) DO UPDATE SET pump_sales_litres=EXCLUDED.pump_sales_litres, theoretical_closing=EXCLUDED.theoretical_closing, variance_litres=EXCLUDED.variance_litres`,
...
app.get('/api/alerts/summary', async (req, res) => {
  try {
    const client = await getDb();
    const result = await client.query(`SELECT severity, COUNT(*) AS count FROM alerts WHERE status = 'open' GROUP BY severity`);
...
app.post('/api/shifts/:id/close', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const result = await closeShift(client, req.params.id, { pumpMeterOpening: pump_meter_opening, pumpMeterClosing: pump_meter_closing, notes });
    res.json(result);
  } catch (err) {
...
app.get('/api/pump-vs-dip', async (req, res) => {
  try {
    const client = await getDb();
    const result = await client.query(
      `SELECT s.id, s.shift_name, s.shift_date, s.opening_nsv, s.closing_nsv, s.pump_meter_sales,
              s.dip_sales, s.variance_litres, s.variance_pct, s.status, s.attendant_name, t.tank_number, t.fuel_type
...
app.post('/api/audit-log', async (req, res) => {
  ...
  try {
    const client = await getDb();
    await client.query(
      `INSERT INTO audit_log (user_email, user_role, action, entity_type, entity_id, station_id, old_value, new_value, ip_address) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
      [user_email, user_role || null, action, entity_type, entity_id || null, station_id || null, old_value ? JSON.stringify(old_value) : null, new_value ? JSON.stringify(new_value) : null, req.headers['x-forwarded-for'] || req.socket.remoteAddress || null]
    );
    res.json({ ok: true });
  } catch (err) {
...
app.get('/api/audit-log', async (req, res) => {
  try {
    ...
    const params = [];
    if (stationId) { params.push(stationId); query += ` WHERE station_id = $${params.length}`; }
    params.push(limit);
...
app.post('/api/payments/initiate', async (req, res) => {
  ...
  if (!station_id || !plan_id || !billing_cycle || !user_email) return res.status(400).json({ error: 'Missing required fields' });
  try {
    const client = await getDb();
    ...
    const payRes = await client.query(`INSERT INTO payments (station_id, amount_kes, billing_cycle, plan_name, status) VALUES ($1,$2,$3,$4,'pending') RETURNING id`, [station_id, amount, billing_cycle, plan.name]);
    const paymentId = payRes.rows[0].id;
    const ipnId = 'ae69c243-c3a9-4717-8932-da50bb3db92b';
    const order = { id: paymentId, currency: 'KES', amount: parseFloat(amount), description: isTest ? 'FuelSense Test Payment' : `FuelSense ${plan.name} - ${billing_cycle} subscription`, callback_url: process.env.FRONTEND_URL + '/payment-success', notification_id: ipnId, billing_address: { email_address: user_email, phone_number: phone || '', country_code: 'KE', first_name: user_name?.split(' ')[0] || 'Customer', last_name: user_name?.split(' ')[1] || '' } };
    ...
    res.json({ payment_id: paymentId, redirect_url: pesapalRes.redirect_url, amount, plan_name: plan.name, billing_cycle, is_test: isTest });
  } catch (err) {
...
app.post('/api/payments/test', async (req, res) => {
  ...
  try {
    ...
    if (!station_id || station_id === 'test' || !uuidRegex.test(station_id)) {
      ...
      if (stationRes.rows.length) { realStationId = stationRes.rows[0].id; }
      else return res.status(400).json({ error: 'No stations found in database' });
    }
    const payRes = await client.query(`INSERT INTO payments (station_id, amount_kes, billing_cycle, plan_name, status) VALUES ($1,$2,'monthly','TEST_PAYMENT','pending') RETURNING id`, [realStationId, amount]);
    const paymentId = payRes.rows[0].id;
    const ipnId = 'ae69c243-c3a9-4717-8932-da50bb3db92b';
    const order = { id: paymentId, currency: 'KES', amount: parseFloat(amount), description: `FuelSense Test Payment - KES ${amount}`, callback_url: process.env.FRONTEND_URL + '/payment-success', notification_id: ipnId, billing_address: { email_address: user_email, phone_number: phone || '', country_code: 'KE', first_name: user_name?.split(' ')[0] || 'Customer', last_name: user_name?.split(' ')[1] || '' } };
    const pesapalRes = await pesapal.submitOrder(order);
    await client.query(`UPDATE payments SET pesapal_order_id = $1 WHERE id = $2`, [pesapalRes.order_tracking_id, paymentId]);
...
app.get('/api/payments/callback', async (req, res) => {
  ...
  try {
    ...
    if (status.payment_status_description === 'Completed') {
      await client.query(`UPDATE payments SET status = 'completed', pesapal_tracking_id = $1 WHERE id = $2`, [OrderTrackingId, OrderMerchantReference]);
      ...
      if (payment && payment.plan_name !== 'TEST_PAYMENT') {
        ...
        if (plan) {
          ...
          if (payment.billing_cycle === 'annual') end.setFullYear(end.getFullYear() + 1); else end.setMonth(end.getMonth() + 1);
          await client.query(
            `INSERT INTO subscriptions (station_id, plan_id, billing_cycle, status, current_period_start, current_period_end) VALUES ($1,$2,$3,'active',$4,$5) ON CONFLICT (station_id, plan_id) DO UPDATE SET status='active', current_period_start=EXCLUDED.current_period_start, current_period_end=EXCLUDED.current_period_end`,
    ...
    res.redirect(`${process.env.FRONTEND_URL}/?tab=payment-result&status=${encodeURIComponent(status.payment_status_description)}&OrderTrackingId=${OrderTrackingId}`);
  } catch (err) {
    console.error('[API] payment callback error:', err.message);
    res.redirect(`${process.env.FRONTEND_URL}/?tab=payment-result&status=Error&error=${encodeURIComponent(err.message)}`);
  }
});
...
app.get('/api/payments/history', async (req, res) => {
  try {
    ...
    const stationId = req.query.station_id;
    if (!stationId) return res.status(400).json({ error: 'station_id required' });
    const result = await client.query(`SELECT * FROM payments WHERE station_id = $1 ORDER BY created_at DESC LIMIT 20`, [stationId]);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/admin/stations', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const result = await client.query(`INSERT INTO stations (id, name, location) VALUES (gen_random_uuid(), $1, $2) RETURNING *`, [name, location || null]);
...
app.put('/api/admin/stations/:id', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const result = await client.query(`UPDATE stations SET name=$1, location=$2 WHERE id=$3 RETURNING *`, [name, location || null, req.params.id]);
...
app.post('/api/admin/tanks', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const result = await client.query(
      `INSERT INTO tanks (id, station_id, tank_number, fuel_type, capacity_litres, fuel_density_at_15c, low_stock_threshold_pct) VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6) RETURNING *`,
...
app.post('/api/admin/users', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const result = await client.query(
      `INSERT INTO user_profiles (supabase_uid, email, full_name, role, station_id) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
...
app.post('/api/admin/suppliers', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const result = await client.query(`INSERT INTO suppliers (id, name, contact_name, phone, email, address, is_active) VALUES (gen_random_uuid(),$1,$2,$3,$4,$5,true) RETURNING *`, [name, contact_name || null, phone || null, email || null, address || null]);
    res.status(201).json(result.rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/admin/suppliers/:id', async (req, res) => {
  ...
  try {
    const client = await getDb();
    const result = await client.query(`UPDATE suppliers SET name=$1, contact_name=$2, phone=$3, email=$4, address=$5, is_active=$6 WHERE id=$7 RETURNING *`, [name, contact_name || null, phone || null, email || null, address || null, is_active !== undefined ? is_active : true, req.params.id]);
...
// ── Start server ──────────────────────────────────────────────────────────
app.listen(PORT, () => { console.log('[API] FuelSense API running on port ' + PORT); });

setInterval(async () => { await checkExpiredSubscriptions(); }, 60 * 60 * 1000);
setInterval(async () => { await checkUpcomingRenewals(); }, 6 * 60 * 60 * 1000);
...
setTimeout(async () => {
  try {
    ...
    async function pollCycle() {
      ...
      let readings;
      try { readings = await getInventory(); } catch (err) { console.error('[scheduler] ATG error:', err.message); return; }
      const client = await getDb();
      for (const reading of readings) {
        try {
          ...
          await client.query(
            `INSERT INTO atg_readings (id, tank_id, recorded_at, innage_mm, water_mm, temperature_c, tov_litres, water_litres, gov_litres, vcf, nsv_litres, is_locked) VALUES (gen_random_uuid(), $1, NOW(), $2, $3, $4, $5, $6, $7, $8, $9, FALSE)`,
          ...
          if (delta > DELIVERY_RISE_THRESHOLD) {
            ...
            if (state.deliveryStatus === 'none') {
              const dRes = await client.query(`INSERT INTO deliveries (id, tank_id, status, offload_started_at) VALUES (gen_random_uuid(), $1, 'in_progress', NOW()) RETURNING id`, [t.id]);
